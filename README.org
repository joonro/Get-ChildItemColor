...
** =ls= equivalent
- Current =Get-ChildItemColor | Format-Wide= is limited in that it can only
  show the same color for a whole row. I'm planning to revert back to the
  previous code for =ls= case only.
* Notes on the code
